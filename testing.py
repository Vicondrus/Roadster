...
def plotStats():
    statsPre = util.makeTop5Stats(".\\output\\germansignsnet3.5eval\\top5.csv")
    ...
    plot.savefig(".\\output\\germansignsnet3.5eval\\classstats.jpg")


def evalModel():
    model = tf.keras.models.load_model(".\\output\\germansignsnet3.5")

    evalX, evalY = util.load_data_and_labels(".\\data\\videoImages", ".\\data\\videoImages\\Eval.csv")
    ...
    util.writeTopToCSV('.\\output\\germansignsnet3.5eval\\top5.csv - custom', top5)

    plot.bar(range(len(stats)), list(stats.values()), align='center')
    plot.xticks(range(len(stats)), list(stats.keys()))
    plot.savefig(".\\output\\germansignsnet3.5eval\\stats - custom.jpg")
